...
const ManageExamQuestions = () => {
  ...
  const handleAddQuestion = async (question) => {
    try {
      ...
      setSuccess('Question added to examination questionnaire.');
    } catch (err) {
  ...
  const handleRemoveQuestion = async (qId) => {
    try {
      ...
      const examRes = await examService.getById(examId);
      setExam(examRes.data);
      setSuccess('Question removed from examination questionnaire.');
    } catch (err) {
  ...
  if (loading) return <Loading message="Loading questions pool..." />;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <div>
          <PageHeader 
            ...
            subtitle={`Editing questionnaire for: "${exam.title}" (${exam.subjectCode})`}
            actions={
              <div className="text-xs font-semibold text-secondary-500">
                Marks: <strong className="text-primary-600">{exam.totalMarks} pts</strong> | Questions: <strong className="text-accent-600">{exam.questions.length}</strong>
              </div>
            }
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start select-none">
        ...
        {/* Inside Exam Pane */}
        <Card title="Questions Linked in Exam" subtitle={`Total linked: ${insideExam.length} prompts`}>
          {insideExam.length === 0 ? (
            <p className="text-xs text-secondary-400 font-medium text-center py-8">
              No questions are added to this exam template yet. Use the right panel to link prompts.
            </p>
          ) : (
              ...
              {insideExam.map((q, idx) => (
                <div key={q.id} className="p-3 border border-secondary-200 bg-white rounded-lg flex items-start gap-3 justify-between">
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-bold text-secondary-400">Question {idx + 1} ({q.marks} pts)</p>
        ...
        {/* Bank Pane */}
        <Card title="Available Subject Bank Questions" subtitle={`Subject pool: ${outsideExam.length} prompts`}>
          {outsideExam.length === 0 ? (
            <p className="text-xs text-secondary-400 font-medium text-center py-8">
              No other questions available in this subject category. All questions have been linked or subject pool is empty.
            </p>
          ) : (
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
              {outsideExam.map((q) => (
                <div key={q.id} className="p-3 border border-secondary-200 bg-white rounded-lg flex items-start gap-3 justify-between">
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-bold text-secondary-400">{q.difficulty} • {q.marks} pts</p>
                    <p className="text-xs font-semibold text-secondary-800 truncate mt-0.5">{q.text}</p>
                  </div>
                  <Button
                    variant="success"
